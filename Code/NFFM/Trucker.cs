...
namespace NFFM
{
    public partial class Trucker : Form
    {
        public Trucker()
        {
            InitializeComponent();
        ...
        {
            String SPName = "Truckers_GetAll";
            DataTable dt = DBManager.GetDataTable(SPName);
            dataGridView1.DataSource = dt;
            dataGridView1.Columns["TruckerID"].Visible = false;
        ...
        private void ShowUpdateInfoDialog() {
            Trucker_AddUpdate add = new Trucker_AddUpdate();
            add.TruckerID = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells[0].Value);
            add.txtTrucker.Text = dataGridView1.SelectedRows[0].Cells[1].Value.ToString();
            add.btnAdd.Text = "Update";
            add.lblAddUpdate.Text = "Update Trucker Information";
            add.ShowDialog();
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                int TruckerID = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells[0].Value);
                String str = System.Configuration.ConfigurationManager.ConnectionStrings["NFFM"].ConnectionString;
                SqlConnection con = new SqlConnection(str);
                SqlCommand cmd = new SqlCommand("Truckers_Delete", con);
